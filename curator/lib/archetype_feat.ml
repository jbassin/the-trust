...
let sourcePatcher : (module Patcher_intf.S with type t = Source.t) =
  Patcher.source_patcher "archetype-feats"
;;
...
let source_of_archetype =
  ...
  fun db feat ->
      ...
      | None ->
        let archetypes =
          ...
          |> List.concat_map ~f:(fun { associated_feats; source; _ } ->
                 List.map associated_feats ~f:(fun feat -> Utils.normalize feat, source))
    ...
    in
    String.Map.find archetypes (Utils.normalize feat)
;;

let rec source_of_prereq =
  let maybe_get_feat key ~db = Db.get db ~brand |> Fn.flip String.Map.find key in
  let _precomp = ref None in
  fun db feat ->
    ...
    | true ->
      ...
      | [] -> None
      | feats ->
        let sourced, unsourced =
          List.partition_tf feats ~f:(fun { source; _ } -> Source.is_missing source)
        in
...
module Param : Runner_intf.S = struct
  ...
  let steps =
    [ (fun ~key:_ ~data ~db ~raw ->
        let (module SourcePatcher : Patcher_intf.S with type t = Source.t) =
          sourcePatcher
        in
